                ...
                <ul>
                    {
                        todos.map((todo) => (
                            <li key={todo.id} className='flex items-center p-3 rounded-lg bg-slate-100 border border-gray-200'>
                                <input type="checkbox" checked={todo.completed} {/* agar todo.complete===true bolsa ishlaydi*/} 
                                onChange={() => setTodos(todos.map((t) => (t.id === todo.id ? { ...t, completed: !t.completed } : t)))}
                                    {/*onchange setTodos bunda checkbox bosilgan shu function ishlaydi setTodos-todos royxati map bu yerda todo ni tekshiradi t- bu todo uchun vaqincha nom  
                                    agar t ni id si todo.id si bilan qattiy teng bolsa tni massivga olib completed ni teskari qiladi yani true-false flase-true ga  */}
                                ...
                                <button onClick={() => setTodos(todos.filter((t) => t.id !== todo.id))}
                                    {/* bosilganda todos.filter - shartga togri kelgan todoni tanlab olib massiv qiladi ,
                                 todo t.id todo.id ga teng bolmasa u massivda qoladi agar teng bolsa ochiriladi . */}
